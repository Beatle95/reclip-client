 ...
 public:
  class Delegate {
   public:
    ...
    virtual void HandleFullSync() = 0;
    virtual void HandleTextSent(bool is_success) = 0;

    virtual void HandleHostData(const HostId& id, const std::string& name) = 0;
    virtual void HandleNewText(const HostId& id, const std::string& text) = 0;
  };

  static std::unique_ptr<ServerConnection> Create(Delegate& delegate);
